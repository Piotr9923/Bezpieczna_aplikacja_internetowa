...
let master_password_strength = document.getElementById("master_password_strength")


let buttons = document.getElementsByTagName("img")

let description = []
description.push("Adres e-mail może składać się z małych i dużych liter alfabetu łacińskiego, cyfr oraz symboli:  znak podkreślenia (_ ), kropka(.), myślnik (-), małpa (@). Maksymalnie 64 znaki.")
description.push("Numer telefonu musi składać się z cyfr. Maksymalnie 12 cyfr.")
description.push("Login może zawierać małe i duże litery alfabetu łacińskiego,cyfry oraz symbole: znak podkreślenia (_ ), kropka(.), myślnik (-). Maksymalnie 32 znaki")
...
description.push("Hasło główne może zawierać małe i duże litery alfabetu łacińskiego,cyfry oraz symbole: znak podkreślenia (_ ), kropka(.), myślnik (-), wykrzyknik (!), dolar ($) i gwiazdkę (*)")
...
function get_password_strength(password){
    ...
    if(popular_passwords.includes(password)){
        return "Bardzo słabe";
    }
